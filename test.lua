...
function Library:addHoverEffects(button, baseColor)  -- New helper function
	local hovered = false
	local down = false
	button.MouseEnter:connect(function()
		hovered = true
		button:tween{BackgroundColor3 = self:lighten(baseColor, 10), Length = 0.15, Style = Enum.EasingStyle.Quad}
	end)
	button.MouseLeave:connect(function()
		hovered = false
		if not down then
			button:tween{BackgroundColor3 = baseColor, Length = 0.15, Style = Enum.EasingStyle.Quad}
		end
	end)
	button.MouseButton1Down:connect(function()
		down = true
		button:tween{BackgroundColor3 = self:lighten(baseColor, 20), Length = 0.1}
	end)
	UserInputService.InputEnded:connect(function(key)
		if key.UserInputType == Enum.UserInputType.MouseButton1 then
			down = false
			button:tween{BackgroundColor3 = (hovered and self:lighten(baseColor, 10)) or baseColor, Length = 0.15}
		end
	end)
end
...
function Library:toggle(options)
	...
	if type(options.StartingState) ~= "boolean" then error("StartingState must be a boolean") end
	...
	self:addHoverEffects(toggleContainer, Library.CurrentTheme.Secondary)
...
function Library:dropdown(options)
	...
	do
		...
		self:addHoverEffects(dropdownContainer, Library.CurrentTheme.Secondary)
...
function Library:button(options)
	...
	self:addHoverEffects(buttonContainer, Library.CurrentTheme.Secondary)
	...
	buttonContainer.MouseButton1Click:connect(function()
		...
		if options.Style == 1 then
			...
			do
				...
				-- Presets
				local presets = holder:object("Frame", {
					AnchorPoint = Vector2.new(0, 1),
					BackgroundTransparency = 1,
				...
				for i, col in ipairs(presetColors) do
					local btn = presets:object("TextButton", {
					...
					btn.MouseButton1Click:connect(function()
						...
						color:tween({BackgroundColor3 = col})
						globalUpdate()
					end)
...
function Library:slider(options)
	...
	if options.Default < options.Min or options.Default > options.Max then error("Default value out of range") end
	...
	self:addHoverEffects(sliderContainer, Library.CurrentTheme.Secondary)
	...
	function methods:Set(value)
		if value < options.Min or value > options.Max then error("Value out of range") end
...
function Library:progressBar(options)
	...
	if options.Value < 0 or options.Value > options.Max then error("Invalid progress value") end

	local progressContainer = self.container:object("Frame", {
		Theme = {BackgroundColor3 = "Secondary"},
		Size = UDim2.new(1, -20, 0, 52)
	}):round(7)

	local text = progressContainer:object("TextLabel", {
		BackgroundTransparency = 1,
		Position = UDim2.fromOffset(10, 5),
		Size = UDim2.new(0.5, -10, 0, 22),
		Text = options.Name,
	...
	local progressBar = progressContainer:object("Frame", {
		Theme = {BackgroundColor3 = {"Secondary", -20}},
		AnchorPoint = Vector2.new(0.5, 1),
	...
	local progressFill = progressBar:object("Frame", {
		Size = UDim2.fromScale(options.Value / options.Max, 1),
		Theme = {BackgroundColor3 = "Tertiary"}
	}):round(5)

	local valueText = progressContainer:object("TextLabel", {
		AnchorPoint = Vector2.new(1, 0),
		Theme = {TextColor3 = "WeakText"},
		Position = UDim2.new(1, -10, 0, 10),
		Size = UDim2.new(0, 50, 0, 20),
		TextSize = 12,
		Text = options.Value .. "/" .. options.Max
	})
	...
	function methods:Set(value)
		if value < 0 or value > options.Max then error("Value out of range") end
		options.Value = value
		progressFill:tween{Size = UDim2.fromScale(value / options.Max, 1)}
		valueText.Text = value .. "/" .. options.Max
		options.Callback(value)
	end
